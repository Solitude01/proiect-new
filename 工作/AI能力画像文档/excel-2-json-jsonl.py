...
def excel_to_json(excel_path, output_json_path=None, format_type='json', selected_columns=None):
    """
    将Excel文件的指定列转换为JSON或JSONL格式
    ...
    if output_json_path:
        if format_type == 'jsonl':
            ...
            print(f"✓ JSONL文件已保存到: {output_json_path}")
        else:
            # 保存为JSON格式
            with open(output_json_path, 'w', encoding='utf-8') as f:
...
class ExcelConverterGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel转JSON/JSONL转换器")
    ...
    def browse_input(self):
        ...
        if filename:
            ...
            # 自动设置输出路径
            path = Path(filename)
            ext = '.jsonl' if self.format_type.get() == 'jsonl' else '.json'
